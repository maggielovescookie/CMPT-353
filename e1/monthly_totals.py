...
def pivot_months_pandas(data):
    ...
    Precip = pd.DataFrame(data) # change the data to dataframe type

    # Add a column 'month' that contains the results of applying the date_to_month function to the existing 'date' column. 
    # [You may have to modify date_to_month slightly, depending how your data types work out. ]
    Precip['month'] = Precip['date'].dt.month #extract the month
    ...
    # Hint: grouped_data.aggregate('sum').reset_index()
    Precip = Precip.groupby(['name','month'])
    PrecipSum= Precip.aggregate('sum').reset_index() 

    # Use the Pandas pivot method to create a row for each station (name) and column for each month.
    monthly = PrecipSum.pivot(index = 'name' , columns = 'month' , values = 'precipitation')
    
    # Repeat with the 'count' aggregation to get the count of observations.
    PrecipCount = Precip.aggregate('count').reset_index()
    counts = PrecipCount.pivot(index = 'name' , columns = 'month' , values = 'precipitation')
...
def main():
    ...
    totals1, counts1 = pivot_months_pandas(data)
    totals1.to_csv('totals1.csv')
